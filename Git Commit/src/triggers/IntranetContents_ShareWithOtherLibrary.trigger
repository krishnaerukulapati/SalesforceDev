...
Reason         :   This trigger fires when contentversion update. 
                   To Share  the contents from Teva Master to other libraries
*********************************************************************************************************************/
trigger IntranetContents_ShareWithOtherLibrary on ContentVersion (after update,before update) {
        ...
        ContentWorkspace contentsDir = null;
        ...
        for(ContentWorkspace workspace : [Select ID from ContentWorkspace Where Name = '*Master' limit 1]){
        //for(ContentWorkspace workspace : [Select ID from ContentWorkspace Where Name like '%Master' limit 1]){
            contentsDir = workspace;
        }
        System.debug('ContentWorkspace Size ::::::::::::::::: '+contentsDir);    
        if(contentsDir != null){
            ...
            for(ContentWorkspaceDoc workspace : [Select Id,ContentDocumentId From ContentWorkspaceDoc
                                                 Where ContentDocumentId IN :documentIds AND ContentWorkspaceId = :contentsDir.id]){
    ...
    if(trigger.IsBefore){
        ...
        for(ContentVersion ver : trigger.new){
          ...
          // If content is loaded to "Teva Master" library, Then validate for share
        ...
        if(!existingContentShared.isEmpty()){
            for(ContentWorkspaceDoc wSpace :[select ContentDocumentId,ContentWorkspaceId,id from ContentWorkspaceDoc where ContentDocumentId in :existingContentShared
                                             and  ContentWorkspaceId  !=:contentsDir.id]){
        ...
        List<string> conditionList = new List<string> ();
            for(ContentVersion ver : tevaMasterContents){
               if(ver.Team__c!=null){
                 teamList = new List<String>();
                 teamList = ver.Team__c.split(';');
                 setLibId.addAll(teamList);
               }
            }
            list<ContentWorkspaceDoc> listContentWorkspaceDoc = new list<ContentWorkspaceDoc >();
            if(tevaMasterContents.size()>0){
            ...
            */
            ...
            if(!listContentWorkspaceDoc.isEmpty() && !Test.isRunningTest()) {
